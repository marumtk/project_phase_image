	...
	input_img_count(0)
{
	...
	stSystemParam.MarkerOffset = MARKER_OFFSET;// (μs) disable : 0(8bit), enable : otherwise(7bit + 1bit)
	stSystemParam.skipMode = PROJ_SKIP_TRIG;// disable: 0, enable: 1
	for( int i = 0; i < 20; i++ ) stSystemParam.LEDAdjust[ i ] = 0;
	for( int i = 0; i < PROJ_VALID_BITS; i++ )
	{
		stSystemParam.BIT_SEQUENCE[i] = BIT_SEQUENCE[i];
		stSystemParam.LEDAdjust[i] = PROJ_LED_ADJUST[i];
	}
...
bool HighSpeedProj::connectProj() {
	...
	iRet = pPcieLibrary->RegisterSGDMA(iBoardID, iChannel, 0, 100 * FRAME_SIZE);
	if (iRet != STATUS_SUCCESSFUL) {
